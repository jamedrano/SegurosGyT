...
def get_outliers_zscore(df, column_name, threshold=3):
    if df is None or df.empty or column_name not in df.columns or df[column_name].isnull().all():
        ...
        return pd.DataFrame(), pd.DataFrame(), np.nan, np.nan
    z_scores = np.abs(zscore(df[column_name].dropna()))
    outliers = df[(z_scores > threshold)]
    return outliers, pd.DataFrame(), np.nan, np.nan
...
def prepare_mi_data(risk_scores_data, listado_data, id_col_listado, target_col_name_in_risk_scores, selected_features_from_listado, bin_target_flag, num_bins_for_target=3):
    if risk_scores_data is None or risk_scores_data.empty or listado_data is None or listado_data.empty:
        logger.error("prepare_mi_data: Risk scores or customer data not available for MI calculation.")
    ...
    if not features_to_process:
        logger.error("prepare_mi_data: No valid features selected/found from ListadoCreditos for MI.")
    ...
    if bin_target_flag:
        ...
        try:
            ...
            logger.error(f"prepare_mi_data: Error binning target '{y_mi_target_name}' for MI: {e}.")
            return None, None, None, None, f"Error binning target '{y_mi_target_name}' for MI: {e}."
    else:
        y_mi = pd.to_numeric(y_mi, errors='coerce').fillna(y_mi.median())
            ...
            if not high_o.empty or not low_o.empty:
                ...
                with pd.ExcelWriter(output_o, engine='xlsxwriter') as writer:
                    if not high_o.empty:
                        ...
                        if 'credito_y' in high_o_details.columns:
                            high_o_details = high_o_details.drop(columns=['credito_y'])
                        if 'credito_x' in high_o_details.columns:
                            high_o_details = high_o_details.rename(columns={'credito_x':'credito'})
                    ...
                    if not low_o.empty:
                        ...
                        if 'credito_y' in low_o_details.columns:
                            low_o_details = low_o_details.drop(columns=['credito_y'])
                        if 'credito_x' in low_o_details.columns:
                            low_o_details = high_o_details.rename(columns={'credito_x':'credito'})
...
with tabs[4]: # Pre-Loan Insights
    ...
    if not uploaded_file:
        ...
        if not available_features_prev_tab:
            st.error("No features from 'ListadoCreditos' for Pre-Loan Insights.")
        else:
                ...
                if not selected_cols_fi_prev_tab:
                    ...
                    with st.spinner("Preparing data and performing pre-loan feature analysis..."):
                        ...
                        if error_message_tab4:
                            ...
                            st.error("Failed to prepare data for Pre-Loan Insights.")
                        else:
